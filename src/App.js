...
import AdminPanel from "./pages/AdminPanel/AdminPanel"; // adjust the path if needed
...
const App = () => {
  ...
  // Call useAdmin hook inside the component
      ...
      <AnimatePresence exitBeforeEnter>
        <Switch location={location} key={location.pathname}>
          <Route
            exact
            path="/admin"
            render={() => (isAdmin ? <AdminPanel /> : <Redirect to="/login" />)}
          />
          <Route exact path="/">
            <Redirect to="/login" />
          </Route>
